...
koji:   clean
	koji -c~/.koji/ai-config build --nowait ai6 git://github.com/cernops/$(PACKAGE).git?#v$(VERSION)-$(RELEASE)
	koji -c~/.koji/ai-config build --nowait ai5 git://github.com/cernops/$(PACKAGE).git?#v$(VERSION)-$(RELEASE)
